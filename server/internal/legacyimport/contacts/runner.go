...
func Run(sourcePath string, dryRun bool, timeout time.Duration) error {
	...
	items, err := loadLegacyContacts(sourcePath)
	if err != nil {
		return err
	}

	log.Printf("discovered %d legacy contacts in %s", len(items), sourcePath)

	if dryRun {
		for _, item := range items {
			log.Printf("dry-run contact=%s client=%q name=%q", item.ID, item.ClientID, item.Name)
		}
	...
	stats, err := importContacts(ctx, db, items)
	if err != nil {
		return err
	}
