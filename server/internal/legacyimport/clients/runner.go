...
func Run(sourcePath string, dryRun bool, timeout time.Duration) error {
	...
	items, err := loadLegacyClients(sourcePath)
	if err != nil {
		return err
	}

	log.Printf("discovered %d legacy clients in %s", len(items), sourcePath)

	if dryRun {
		for _, item := range items {
			log.Printf("dry-run client=%s title=%q region=%q", item.ID, item.Title, item.RegionID)
		}
	...
	stats, err := importClients(ctx, db, items)
	if err != nil {
		return err
	}
