...
var canonicalTicketStatuses = []ticketLookup{
	{Type: "assigned", Title: "назначен"},
	{Type: "cancelled", Title: "отменен"},
	{Type: "closed", Title: "закрыт"},
	{Type: "created", Title: "создан"},
	{Type: "inWork", Title: "в работе"},
	{Type: "worksDone", Title: "работы завершены"},
}
...
func loadLegacyTicketStatuses(path string) ([]ticketLookup, error) {
	if _, err := os.Stat(path); err != nil {
		return nil, fmt.Errorf("stat dump: %w", err)
	}

	items := make([]ticketLookup, len(canonicalTicketStatuses))
	copy(items, canonicalTicketStatuses)
	return items, nil
}

func importTicketStatuses(ctx context.Context, db *pgxpool.Pool, items []ticketLookup) (int, error) {
	if err := ensureTicketStatusesSchema(ctx, db); err != nil {
		return 0, err
	}
	...
	for _, item := range items {
		if _, err := db.Exec(
			ctx,
			`INSERT INTO ticket_statuses (type, title)
			 VALUES ($1, $2)
			...
			return imported, fmt.Errorf("import ticket status %s: %w", item.Type, err)
		}
		imported++
	}
	...
	if _, err := db.Exec(ctx, `DELETE FROM ticket_statuses WHERE NOT (type = ANY($1))`, allowed); err != nil {
		return imported, fmt.Errorf("prune ticket statuses: %w", err)
	}

	return imported, nil
}

func ensureTicketStatusesSchema(ctx context.Context, db *pgxpool.Pool) error {
	if _, err := db.Exec(
		ctx,
		...
		return fmt.Errorf("ensure ticket_statuses table: %w", err)
	}

	if _, err := db.Exec(ctx, `ALTER TABLE ticket_statuses ADD COLUMN IF NOT EXISTS title TEXT NOT NULL DEFAULT ''`); err != nil {
		return fmt.Errorf("ensure ticket_statuses.title column: %w", err)
	}
