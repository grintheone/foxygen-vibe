...
export function EditorClientsPage() {
    ...
    const selectedRegionTitle =
        regions.find((region) => region.id === formState.region)?.title ||
        selectedClient?.regionTitle?.trim() ||
        "Не указан";
    ...
    return (
        <PageShell>
            <section className="w-full space-y-6">
                <EditorPageHeader title="Клиенты" leadingAction={<BackButton onClick={handleBack} />} />
                    ...
                    sidebar={
                        ...
                        >
                            <div className="space-y-4">
                                ...
                                <EditorListError
                                    error={clientsError}
                                    fallbackMessage="Не удалось загрузить список клиентов."
                                />
                            </div>
                ...
                >
                    <EditorPane editorPaneRef={editorPaneRef}>
                        {!selectedClientId ? (
                            <EditorNoticeCard
                                dashed
                                message="Выберите клиента слева, чтобы открыть карточку редактора."
                            />
                        ) : null}

                        {selectedClientId && isClientLoading ? (
                            <EditorNoticeCard message="Загружаем карточку клиента..." />
                        ) : null}

                        {selectedClientId && selectedClientError ? (
                            <EditorListError
                                error={selectedClientError}
                                fallbackMessage="Не удалось загрузить карточку клиента."
                            />
                        ) : null}
                                        ...
                                        footer={
                                            <>
                                                <p className="text-xs text-slate-500">
                                                    ЛИС и менеджеры пока доступны только для просмотра. Координаты пока
                                                    собираются вручную через широту и долготу{" "}
                                                </p>
                                                {regionsError ? (
                                    ...
                                    >
                                        <EditorContextSection title="Основное">
                                            ...
                                            <EditorContextItem
                                                ...
                                                value={selectedClient.activeAgreementCount}
                                            />
                                            <EditorContextItem label="Регион" value={selectedRegionTitle} />
                                            <EditorContextItem
                                                label="ЛИС"
                                                value={selectedClient.laboratorySystem || "Не привязана"}
                                            />
                                            <EditorContextItem
                                                label="Менеджеров"
                                                value={selectedClient.manager?.length || 0}
                                            />
                                        </EditorContextSection>
                                        ...
                                        <EditorContextSection title="Координаты">
                                            <EditorContextItem
                                                label="Широта"
                                                value={formState.latitude || "Не указана"}
                                            />
                                            <EditorContextItem
                                                label="Долгота"
                                                value={formState.longitude || "Не указана"}
                                            />
                                        </EditorContextSection>
                                    </EditorContextPanel>
                                </section>
                                ...
                                <p className="text-xs text-slate-500">
                                    {isClientFetching
                                        ...
                                        : isRegionsFetching
                                          ...
                                          : "Изменения применяются к базовым полям клиента, региону и координатам"}
                                </p>
                            </>
                        ) : null}
                    </EditorPane>
