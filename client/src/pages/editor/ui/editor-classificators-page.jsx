...
function ClassificatorListItem({ classificator, isActive, onClick }) {
    ...
    const meta = [classificator.manufacturerTitle?.trim(), classificator.researchTypeTitle?.trim()]
        .filter(Boolean)
        .join(" • ");
            ...
            <div className="flex items-start justify-between gap-4">
                <div className="space-y-2">
                    ...
                    <p className="text-sm text-slate-400">
                        {meta || "Производитель и тип исследования пока не указаны."}
                    </p>
                </div>
    ...
    return (
        <PageShell>
            <section className="w-full space-y-6">
                <EditorPageHeader title="Классификаторы" leadingAction={<BackButton onClick={handleBack} />} />
                        ...
                        <EditorSidebar
                            ...
                            footer={
                                isClassificatorsFetching
                                    ? "Обновляем список..."
                                    : `Показано ${classificators.length} записей.`
                            }
                        >
                            <div className="space-y-4">
                                ...
                                <EditorListError
                                    error={classificatorsError}
                                    fallbackMessage="Не удалось загрузить список классификаторов."
                                />
                            </div>

                            <div className="min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
                                {isClassificatorsLoading ? (
                                    <EditorNoticeCard message="Загружаем классификаторы..." />
                                ) : null}
                ...
                >
                    <EditorPane editorPaneRef={editorPaneRef}>
                        {!selectedClassificatorId ? (
                            <EditorNoticeCard
                                dashed
                                message="Выберите классификатор слева, чтобы открыть карточку редактора."
                            />
                        ) : null}
                        ...
                        {selectedClassificatorId && selectedClassificatorError ? (
                            <EditorListError
                                error={selectedClassificatorError}
                                fallbackMessage="Не удалось загрузить карточку классификатора."
                            />
                        ) : null}
                            ...
                            <>
                                ...
                                <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
                                    <div className="space-y-5 rounded-3xl border border-white/10 bg-white/5 p-5">
                                        ...
                                        <EditorFormField label="Регистрационное удостоверение">
                                            <textarea
                                    ...
                                    >
                                        <EditorContextSection title="Основное">
                                            <EditorContextItem
                                                label="Производитель"
                                                value={selectedManufacturerTitle}
                                            />
                                            <EditorContextItem
                                                label="Тип исследования"
                                                value={selectedResearchTypeTitle}
                                            />
                                            <EditorContextItem
                                                label="Связанных устройств"
                                                value={selectedClassificator.deviceCount}
                                            />
                                    ...
                                    {isClassificatorFetching
                                        ...
                                        : isManufacturersFetching
                                          ...
                                          : isResearchTypesFetching
                                            ...
                                            : "Изменения применяются к полям классификатора, не затрагивая конкретное оборудование"}
                                </p>
                            </>
                        ) : null}
                    </EditorPane>
