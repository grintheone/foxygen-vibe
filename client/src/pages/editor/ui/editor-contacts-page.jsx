        ...
        >
            <div className="space-y-2">
                ...
                <p className="text-xs text-slate-500">
                    {contact.phone?.trim() || contact.email?.trim() || "Без контактов"}
                </p>
            </div>
        </button>
    );
...
export function EditorContactsPage() {
    ...
    const selectedClientTitle =
        clientOptions.find((client) => client.id === formState.client)?.title ||
        selectedContact?.clientName?.trim() ||
        "Не выбран";
    ...
    return (
        <PageShell>
            <section className="w-full space-y-6">
                <EditorPageHeader title="Контакты" leadingAction={<BackButton onClick={handleBack} />} />
                    ...
                    sidebar={
                        ...
                        >
                            <div className="space-y-4">
                                ...
                                <EditorListError
                                    error={contactsError}
                                    fallbackMessage="Не удалось загрузить список контактов."
                                />
                            </div>
                ...
                >
                    <EditorPane editorPaneRef={editorPaneRef}>
                        {!selectedContactId ? (
                            <EditorNoticeCard
                                dashed
                                message="Выберите контакт слева, чтобы открыть карточку редактора."
                            />
                        ) : null}

                        {selectedContactId && isContactLoading ? (
                            <EditorNoticeCard message="Загружаем карточку контакта..." />
                        ) : null}

                        {selectedContactId && selectedContactError ? (
                            <EditorListError
                                error={selectedContactError}
                                fallbackMessage="Не удалось загрузить карточку контакта."
                            />
                        ) : null}
                            ...
                            <>
                                ...
                                <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
                                    <div className="space-y-5 rounded-3xl border border-white/10 bg-white/5 p-5">
                                        ...
                                        <EditorFormField
                                            label="Клиент"
                                            hint="Контакт обязательно должен быть привязан к клиенту."
                                        >
                                            <div className="mt-3">
                                                <SelectField
                                                    name="client"
                                    ...
                                    >
                                        <EditorContextSection title="Основное">
                                            ...
                                            <EditorContextItem
                                                label="Должность"
                                                value={formState.position?.trim() || "Не указана"}
                                            />
                                            <EditorContextItem
                                                label="Телефон"
                                                value={formState.phone?.trim() || "Не указан"}
                                            />
                                            <EditorContextItem
                                                label="Email"
                                                value={formState.email?.trim() || "Не указан"}
                                            />
                                        </EditorContextSection>
                                    </EditorContextPanel>
                                </section>
