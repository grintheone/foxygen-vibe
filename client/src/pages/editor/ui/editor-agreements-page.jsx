...
export function EditorAgreementsPage() {
    ...
    useLoadedEditorRecord({
        ...
        onRecordLoad: (agreement) => {
            ...
            setSelectedDeviceOption(
                createDeviceSelection(agreement.device, agreement.deviceTitle, agreement.deviceSerialNumber),
            );
            setFeedback({
                message: "",
                tone: "idle",
    ...
    async function handleSave() {
        ...
        try {
            ...
            setSelectedActualClientOption(
                createClientSelection(updatedAgreement.actualClient, updatedAgreement.actualClientName),
            );
            setSelectedDistributorOption(
                createClientSelection(updatedAgreement.distributor, updatedAgreement.distributorName),
            );
            setSelectedDeviceOption(
                createDeviceSelection(
                    updatedAgreement.device,
                    updatedAgreement.deviceTitle,
                    updatedAgreement.deviceSerialNumber,
                ),
            );
            setFeedback({
    ...
    return (
        <PageShell>
            <section className="w-full space-y-6">
                <EditorPageHeader title="Договоры" leadingAction={<BackButton onClick={handleBack} />} />
                ...
                <EditorWorkspace
                    sidebar={
                        <EditorSidebar
                            ...
                            footer={
                                isAgreementsFetching ? "Обновляем список..." : `Показано ${agreements.length} записей.`
                            }
                        >
                  ...
                  placeholder="Номер, клиент, дистрибьютор, оборудование"
                                ...
                                <EditorListError
                                    error={agreementsError}
                                    fallbackMessage="Не удалось загрузить список договоров."
                                />
                            </div>
                ...
                >
                    <EditorPane editorPaneRef={editorPaneRef}>
                        {!selectedAgreementId ? (
                            <EditorNoticeCard
                                dashed
                                message="Выберите договор слева, чтобы открыть карточку редактора."
                            />
                        ) : null}

                        {selectedAgreementId && isAgreementLoading ? (
                            <EditorNoticeCard message="Загружаем карточку договора..." />
                        ) : null}

                        {selectedAgreementId && selectedAgreementError ? (
                            <EditorListError
                                error={selectedAgreementError}
                                fallbackMessage="Не удалось загрузить карточку договора."
                            />
                        ) : null}
                            ...
                            <>
                                <EditorRecordHeader
                                    ...
                                    title={
                                        selectedAgreement.number
                                            ? `Договор #${selectedAgreement.number}`
                                            : "Договор без номера"
                                    }
                                ...
                                <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
                                    <div className="space-y-5 rounded-3xl border border-white/10 bg-white/5 p-5">
                                        <EditorFormField
                                            label="Фактический клиент"
                                            hint="Основная привязка договора. Поле обязательно."
                                        >
                                            <div className="mt-3">
                                                <AsyncSearchSelect
                                                    ...
                                                    getOptionDescription={(client) =>
                                                        client.address?.trim() || client.regionTitle?.trim() || ""
                                                    }
                                                    ...
                                                    disabled={
                                                        isActualClientOptionsLoading && actualClientOptions.length === 0
                                                    }
                                                    isLoading={
                                                        isActualClientOptionsLoading || isActualClientOptionsFetching
                                                    }
                                                    errorMessage={
                                        ...
                                        <EditorFormField label="Дистрибьютор">
                                            <div className="mt-3">
                                                <AsyncSearchSelect
                                                    ...
                                                    disabled={
                                                        isDistributorOptionsLoading && distributorOptions.length === 0
                                                    }
                                                    isLoading={
                                                        isDistributorOptionsLoading || isDistributorOptionsFetching
                                                    }
                                                    errorMessage={
                                    ...
                                    >
                                        ...
                                        <EditorContextSection title="Статус">
                                            <EditorContextItem
                                                label="Активность"
                                                value={formState.isActive ? "Активный" : "Неактивный"}
                                            />
                                            <EditorContextItem
                                                label="Гарантия"
                                                value={formState.onWarranty ? "Да" : "Нет"}
                                            />
                                            <EditorContextItem
                                                label="Начало"
                                                value={formatAgreementDateTime(formState.assignedAt)}
                                            />
                                            <EditorContextItem
                                                label="Завершение"
                                                value={formatAgreementDateTime(formState.finishedAt)}
                                            />
                                        </EditorContextSection>
                                    </EditorContextPanel>
                                </section>
